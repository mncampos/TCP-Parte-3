...
public class Database {
	...
	public Database() {
		...
		// Atualiza as conferências devido ao relacionamento circular entre Artigos e
		// Conferencias
	...
	private Map<Integer, Pesquisador> initPesquisadores() {
		...
		TopicosList.add(1);
		TopicosList.add(0);
		Pesquisador p1 = new Pesquisador(id++, "João", AfiliacoesMap.GetUniversidade(0),
	...
	public void printConferencias() {
		for (int ind = 1; ind <= this.conferencias.size(); ind++) {
			...
			System.out.println("Conferência = " + this.conferencias.get(ind).getSigla() + "\n" + "Membros = "
	...
	public void printArtigos() {
		for (int ind = 1; ind <= this.artigos.size(); ind++) {
			System.out.println("Nome do Artigo = " + this.artigos.get(ind).getTitulo() + "\n" + "Autor = "
					...
					+ this.artigos.get(ind).getConferência().getSigla() + "\n" + "Tópico = "
